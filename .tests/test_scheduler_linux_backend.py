...
class TestLinuxBackendUnitGeneration:
    ...
    def test_install_creates_service_and_timer(self):
        ...
        service_path = Path(self.systemd_dir) / "ailaunchpad-scheduler-test-task.service"
        timer_path = Path(self.systemd_dir) / "ailaunchpad-scheduler-test-task.timer"
    ...
    def test_service_contains_wrapper_path(self):
        ...
        service_content = (
            Path(self.systemd_dir) / "ailaunchpad-scheduler-exec-test.service"
        ).read_text()
    ...
    def test_timer_contains_oncalendar(self):
        ...
        timer_content = (
            Path(self.systemd_dir) / "ailaunchpad-scheduler-cal-test.timer"
        ).read_text()
    ...
    def test_timer_weekday_schedule(self):
        ...
        timer_content = (
            Path(self.systemd_dir) / "ailaunchpad-scheduler-weekday-test.timer"
        ).read_text()
    ...
    def test_uninstall_removes_files(self):
        ...
        service_path = Path(self.systemd_dir) / "ailaunchpad-scheduler-rm-test.service"
        timer_path = Path(self.systemd_dir) / "ailaunchpad-scheduler-rm-test.timer"
